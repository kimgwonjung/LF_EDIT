...
def make_config():
    ...
    # DATA
    parser.add_argument('--train_path', type=str, default="./MI_TRAIN_TEST.csv")
    parser.add_argument('--val_path', type=str, default="./MI_VAL_TEST.csv")
    parser.add_argument('--inference_path', type=str, default="./")
    parser.add_argument('--output_path', type=str, default="/workspace/Final_DATA_Output")
...
def main_worker(gpu, ngpus_per_node, args):
    ...
    scheduler = optim.lr_scheduler.CosineAnnealingLR(optimizer, T_max=data_length / args.batch_size, eta_min=0)
...
dir_v = {"up_left" : [2,1,0,-1], "up_right": [2,1,0,-1], "down_left": [1,0,-1,-2], "down_right":[1,0,-1,-2]}
dir_u = {"up_left" : [2,1,0,-1], "up_right": [1,0,-1,-2], "down_left": [2,1,0,-1], "down_right":[1,0,-1,-2]}
...
def train(train_loader, model, criterion, optimizer, epoch, args, gpu):
    ...
    for idx, items in enumerate(train_loader):
        ...
        cnt = 0
        if epoch < 3:
            angular_range = [[3, 3, "down_right"]]
        else:
            angular_range = [[3, 3, "down_right"], [2, 2, "up_left"], [2, 4, "up_right"], [4, 2, "down_left"], [4, 4, "down_right"]]
        ...
        for angular_index in angular_range:
            ...
            with torch.no_grad():
                ...
                n_disp = disparity_norm(disp, DISP).detach()

            target_LF = propagation(target_img, n_disp, angular_index[2])

            gt_LF = GT[:,:,angular_index[0]-dir_v[angular_index[2]][0]:angular_index[0]-dir_v[angular_index[2]][-1]+1, angular_index[1]-dir_u[angular_index[2]][0]:angular_index[1]-dir_u[angular_index[2]][-1]+1].clone()
            back_LF = BACK[:,:,angular_index[0]-dir_v[angular_index[2]][0]:angular_index[0]-dir_v[angular_index[2]][-1]+1, angular_index[1]-dir_u[angular_index[2]][0]:angular_index[1]-dir_u[angular_index[2]][-1]+1].clone()
            IMG_L1_Loss = 0.0
            IMG_Perc_Loss = 0.0

            for s_y in range(0, 512, 128):
                for s_x in range(0, 1024, 128):
                    ...
                    crop_intermediate_LF_Output = rearrange(crop_intermediate_MI_Output, "B C (H V) (W U) -> B C V U H W", V=4, U=4)
                    ...
                    Output[:, :, angular_index[0]-dir_v[angular_index[2]][0]:angular_index[0]-dir_v[angular_index[2]][-1]+1, angular_index[1]-dir_u[angular_index[2]][0]:angular_index[1]-dir_u[angular_index[2]][-1]+1, s_y:s_y + 128, s_x:s_x + 128]  = crop_intermediate_LF_Output.detach()
    ...
    with torch.no_grad():
        ...
        for idx, items in enumerate(val_loader):
            ...
            if epoch < 5:
                ...
                angular_range = [[3, 3, "down_right"], [2, 2, "up_left"], [2, 4, "down_left"], [4, 2, "up_right"],
                                 [4, 4, "down_right"]]
...
def propagation(target, target_disp,dir=None):
    b,c,h,w = target.shape

    temp = torch.zeros((b, c, 4, 4, h, w)).cuda()
    ...
    for i, v in enumerate(dir_v[dir]):
        for ii, u in enumerate(dir_u[dir]):
